...
!!! question "Почему для каждого цвета используется именно диапазон от 0 до 255?"
...
!!! question "Почему геометрическая интерпретация помогает сравнивать цвета?"
...
!!! question "Как бы вы вычислили расстояние между цветами `(255, 10, 5)` и `(210, 0, 0)`?"
    

!!! question "Как подсчет расстояния между цветами поможет нам сделать pixel art?"
...
!!! note "Куда устанавливать?"
    Так как minecraft использует python без виртуального окружения, 
    то надо будет выполнить команду в командной строке windows, а не pycharm. 
    Иначе minecraft не увидит установленные пакет
