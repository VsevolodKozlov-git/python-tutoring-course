...
- Для вашего приложения напишите переиспользуемые компоненты:
    - Обязательно используйте `props`
    - Обязательно используйте `slots`, причем так, чтобы хотя бы в одном компоненте было несколько `slots`
- Сделайте отдельный `view`, в котором по нажатии кнопки будет отсылаться запрос на API: [ссылка на API](https://meowfacts.herokuapp.com/). 